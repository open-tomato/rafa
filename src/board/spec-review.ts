 ...
 * plan session opens its answer with, read out of the session's
 * captured output.
 *
 ...
 * plan prompt asks the session to judge that before planning and to
 * open its answer with
 *
 * ````markdown
 * ```rafa:spec-review
 ...
 *
 * Only the FIRST `rafa:spec-review` block counts, the opposite of
 * `report/parse.ts`, which reads the LAST `rafa:report`. The two
 * prompts ask for opposite things: a report ENDS the final message, so
 * anything before it is a draft, while the review OPENS the answer and
 * is written before the plan exists. A session that goes on to quote
 * the review format while explaining the plan writes a second block
 * after the real one, and reading the last would answer the quotation.
 *
...
export interface SpecReviewReading {
  ...
  /** The first `rafa:spec-review` block, or null when there is none. */
...
/**
 * Reads the first `rafa:spec-review` block out of a plan session's
 * captured output.
 *
...
export function parseSpecReview(output: string): SpecReviewReading {
  const block = readRafaBlocks(output).find((found) => found.kind === SPEC_REVIEW_KIND);
  if (block === undefined) {
