 ...
 * anything is read, and which action is asked about whatever a list
 * says.
 *
 ...
 *
 * A list naming `ready` is refused, and so is a list naming a word that
 ...
 *
 * {@link readYesCeiling} refuses `ready` in a list, and
 * {@link allowedUnasked} answers false for it whatever the ceiling
 * holds. The first is the enforcement for every line a person types;
...
export const ALWAYS_ASKED: NextActionId = 'ready';
...
function readCeilingId(word: string, usage: string): NextActionId {
  if (word === ALWAYS_ASKED) {
    throw ceilingRefusal(
      `--${YES_FLAG} names ${ALWAYS_ASKED}, which no list runs unasked: marking an issue ready`
 ...
 *
 * Throws `CommandExit(2, ...)` naming `usage` for a list holding
 * `ready` and for one holding a word that is no action id; see the
 * module note.
 */
 ...
 * ceiling names it, and never for {@link ALWAYS_ASKED}, whatever the
 * ceiling holds. A null ceiling is no `--yes` at all, where every
