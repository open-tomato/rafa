...
export default wrapPhaseZeroCommand({
  ...
  description: 'Hands the spec to a Claude Code session that writes `PLAN-<stub>.md` in `plan.dir`,'
    ...
    + ' spec needs setup no task can do. The stub is the basename of the spec unless `--stub` names one,'
    + ' and the command refuses when that plan already exists. The findings in `progress.txt` are handed'
    + ' to the session as advisory context unless `--no-progress` is given. The session loads the setting'
    + ' sources `loop.settingSources` names, and a config the loop cannot run on refuses the command before'
    + ' any session starts.',
  args: [],
  flags: [
    {
