...
model: sonnet
provenance:
  origin: "https://github.com/affaan-m/everything-claude-code (agents/code-reviewer.md)"
  license: "MIT"
...
3. **Read surrounding code** — Don't review changes in isolation. Read the full file and understand imports, dependencies, and call sites. In TypeScript repos, trace call sites with `ts-symbols` (`outline <file>`, `def|refs|type <file>:<line>:<col>`; see `~/.claude/skills/ts-symbols-for-agents/SKILL.md`) instead of grepping overloaded or re-exported names.
...
When reviewing code in this project, ensure all gates pass before approving:
...
Do not approve code that fails any gate. The gates must all pass as part of the code review process.
