...
- CLI: `rafa loop start --skills-resolver=planner|tag|none` overrides `task.skills` for one run, and a value outside the three is refused.
- Plan: `rafa plan` gives the planner an index of the skills a loop session will see, after the routing table, one line per skill cut at 160 characters and the whole cut at 8,000 with a `N more: rafa skill list` tail; the bundled `dev-planner` skill tells the planner to name a task's skills with `skills=`, only from that index, and that naming none is a valid answer; `rafa plan validate` and `rafa loop start` refuse a `skills=` name no loaded tier resolves (held by no tier, switched off, or only in an unloaded tier), naming the task line and the fix.
- Loop: each task's prompt carries `## Skills for this task`, chosen by the run's resolver (`planner` takes the task's `skills=`, `tag` ranks the served skills against the task text without a model and keeps the top 3, `none` hands nothing), and, with `task.lessons` on, `## Lessons from earlier tasks`, up to five blessed lessons whose file path the task names or whose trigger matches its text, highest confidence first; both go after the blocker line and before `PROMPT.md`, an empty one is left out so a task with nothing to hand out gets the prompt it got before, the resolver is never named in the prompt, and a failed lessons pull is a warning that dispatches the task without the lessons section.
- Effort: each `dispatches` row records the resolver a task ran under and the skills and lesson ids it was offered; a new SQLite-only `skill_invocations` table counts each skill a session invoked, in its main thread and its subagents, reading only the `Skill` tool's name and `skill` input, and a log written by another Claude Code version is recorded as an `unknown` count, never 0; `rafa effort collect` counts the skills of the sessions it appends, and `--skills` counts those of sessions the store already holds.
- Doctor: `rafa doctor` warns when the installed Claude Code is not the version the skill-use collector reads, since skill counts for its sessions are stored as unknown.
- Docs: `docs/skills-measurement.md` fixes, before any measured plan starts, the unit, the two measures (skill references per plan and uptake of offered skills), the before and after conditions, the reading decided in advance and the post-merge step that fills the before table; `context/effort-store.md` covers the new columns, the `skill_invocations` table and its unknown count, the collector's exception to the "no tool input" rule and the moved table-list expectations; `context/workflow.md` and the README's "Which agents and skills are involved" cover the skill index, the `skills=` refusal, the three resolvers, the two prompt sections and the two settings.

## 0.19.0 — 2026-09-25, rafa learns from its own runs

- Learning: a new pure library, published as the `@open-tomato/rafa/learning` subpath and loading under bun and node, settles conflicting lessons by fixed rules — one action confirmed by several sources becomes one lesson whose `usage_count` counts the distinct sources, an action more than 0.10 below its trigger's leader is discarded, and near-equal actions are flagged — blesses the ones tasks may use and names the ones that recurred enough to promote; the local adapter merges each push into the `.md` instincts under `.rafa/instincts/`, logs every merge decision to `instincts.ndjson`, and hands tasks the project's blessed lessons most trusted first plus `~/.rafa/instincts` lessons on triggers the project holds none on, never writing the user scope; instinct files gain the optional `sources` and `promoted_to` fields.
- Loop: each task's lessons (its findings that carry a `resolution`) are pushed to the configured learning adapter after its report is stored, and a refused push is a warning that does not fail the task; the wrap-up session is handed the lessons that recurred enough to promote and answers each in a `rafa:promoted` block instead of reading `progress.txt`, lessons it wrote into a page it actually changed are marked promoted and stop being injected, and the PR body names each listed lesson left unanswered or unchanged.
- Report: a finding can name its `domain`, and a domain outside the known set is reported as an issue.
- Config: `learning.bless.minConfidence` (default 0.5), `learning.promote.after` (default 3) and `learning.promote.minConfidence` (default 0.7), which `rafa init` writes commented out; a confidence outside 0.3 to 0.9 or an `after` below 1 is refused.
- CLI: new `rafa instinct flag <id> <reason>` marks a held lesson as wrong so no later bundle blesses it, refusing an id the project does not hold; new `rafa instinct promote` prints the promotable lessons and writes nothing; `rafa instinct list --blessed` lists the lessons a task would be handed and `--conflicts` shows each flagged trigger's actions side by side with their confidence and usage; the root help says the `instinct` subject can flag and promote.
- Docs: `context/learning.md` covers the library and its rules, the conversion from finding to lesson, the adapter, the wrap-up's promotion check and the config keys, listed in `AGENTS.md`; the README's "Shared learning" paragraph says what ships and the Roadmap line is ticked.

## 0.18.0 — 2026-09-25, Skill tiers — project → rafa → user, served per session

- Tiers: rafa resolves every skill and agent name in one fixed order, project → rafa → user, counting the user tier only when `loop.settingSources` includes `user` and the rafa tier unless `tiers.rafa: off`; two different copies of one name in loaded tiers are a `collision`, and `rafa loop start` and `rafa plan validate` refuse a plan that routes to one with `agent=` or names one with `skills=`, or names an item no loaded tier holds or one switched off with `false`, before any session starts, naming both paths and the one pin line that settles it; byte-identical copies (after removing rafa's `vendored by rafa` header) count as one item served by the nearer tier; a skill pin naming a copy Claude Code would not load is refused (a `rafa` pin while the project or a loaded user tier holds a different copy, a `project` pin while a loaded user tier does), and the suggested pin names the copy Claude Code loads.
- Loop: each task and wrap-up session is served rafa's own skills and agents from a per-run copy under `.rafa/runs/<run>/served/`, through session-only flags (`--agents` and `--add-dir`), without writing anything under the project's `.claude/`; unreviewed third-party items are never served, and rafa's `bundled/bin` goes at the front of the session's `PATH`.
- Config: `tiers.rafa` (`on` or `off`), `tiers.skills` and `tiers.agents` (a name mapped to `false` or to the `project`, `rafa` or `user` tier) and `routing` (a task shape mapped to its agent, defaulting to rafa's five routes) are new settings; the maps merge by key across the user and project config, `false` removes an item, a user-level pin naming an unloaded tier warns instead of failing, and `rafa init` writes all four commented out at their defaults.
- Skills and agents: rafa ships its own tier under `bundled/skills`, `bundled/agents` and `bundled/bin` beside its entry: six agents (`loop-implementer`, `doc-updater`, `tdd-guide`, `build-error-resolver`, `code-reviewer` and `qa-bug-reporter`) and the `documentation`, `git-workflow`, `dev-planner` and `ts-symbols` skills; skills and agents may declare `provenance` as `first-party` or `{ origin, license, reviewed }`, the checkers refuse a malformed value and an agent without the `name` and `description` Claude Code needs, and `NOTICE` documents the redistributed third-party agents.
- CLI: `rafa init` writes nothing under `.claude/` and no longer suggests vendoring an agent rafa ships; `rafa agent vendor` copies from the rafa tier as well as `~/.claude/agents` and says which tier it copied from; `rafa skill list` and `rafa agent list` show `source: rafa` rows as visible to the loop and take `--state=collision`; `rafa doctor` lists every tier collision at once with its pin line, unreviewed third-party items, deletable byte-identical copies, user items without provenance and a Claude Code version other than the one serving was measured on, and the missing-agent refusal names the tier and setting or the pin instead of always suggesting `rafa agent vendor`.
- Plan: `rafa plan` shows the planner this project's `routing` setting as a shape-to-agent table, reads its plan format from the bundled `dev-planner` skill, and a plan routing to `loop-implementer` validates in a freshly initialised repository.
- Tools: rafa ships `ts-symbols` as an executable at `dist/bundled/bin/ts-symbols`; it uses the project's own `typescript`, found from the working directory like `tsc`, and exits 3 with `no typescript under <root>: add it to the project` when there is none; `rafa plan needs` and `rafa doctor --deep` read it as present before `PATH`.
- Fixed: the agent roster follows symbolically linked agent definitions and skips links that point nowhere.
- Docs: `context/inventory.md` covers the tiers, the resolver, collisions, the byte-identical rule, serving and provenance, and the README's "Which agents and skills are involved" covers the rafa tier, pins and `false`.

## 0.17.0 — 2026-09-24, `rafa status` — everything in one snapshot, and one line about what changed

- CLI: new top-level `rafa status` prints where the project stands in five sections — the branch and its plan with task counts, running and paused loops with every `[BLOCKED]` task, the branch's pull request with its checks and mergeability, the roadmap's next line with the open blocked-issue count, and what `rafa cleanup` would list — in code and starting no session. The pull request and board sections, and the merged-pull listing behind housekeeping, each read under a 5-second deadline; one that fails, times out or has no `gh` provider is shown as one `warn` line and `{ "read": false, "problem": ... }` under `--output=json`, and the command still exits 0, exiting 1 only when the config cannot be used. Before any command that runs inside a project, rafa now compares a local reading (git without fetching, the session records, the plan trackers) with the one it saved after the previous command in `.rafa/status-seen.json`, and prints one stderr line when a worktree went idle, a merged branch was left behind, or a loop stopped or blocked, naming `rafa cleanup` or `rafa status`. It never asks or waits, is never printed under `--output=json`, and says nothing on the first run.
- Config: `status.notice` (default `true`, written commented out by `rafa init`) set to `false` turns the since-last-command notice off and stops the snapshot being written.
- Internals: the `gh` runner takes an optional timeout, after which it kills `gh` and answers a failure naming the timeout.

## 0.16.0 — 2026-09-24, References in specs and bugs are verified and stamped

- Plans from the board: `rafa plan create --issue` and `--next` add check 4 to the readiness gate. Every reference a spec names (an issue, a file, an exported symbol, a command, a flag, a config key) is checked against its target and against the fingerprint stamped when the spec was read, after the snapshot settles and before any planning session starts. A `dangling` or `suspect` reference is refused with exit 2, printed as `dangling <text> (line <n>)` or `suspect #<n>: heading "<h>" changed`, while `resolved` blockers (naming `rafa issue unblock <n>`) and unreadable cross-repository `unknown` references are listed and let through. `--accept-refs` re-stamps the references as reviewed and plans. The stamps live in a `rafa:refs` block at the top of the saved copy, which is ignored when checking whether the issue changed and is kept through a refresh or a notes rebuild. `--dry-run` writes no saved copy, so it skips the check.
- Config: `dangerous.acceptStaleRefs` (default `false`, written commented out by `rafa init`) accepts dangling and suspect references on every board run, and prints a warning first in the `plan create` run.
- CLI: `rafa issue check <n> [--stamp]` lists every reference in a spec's saved copy with its state, exits 0 whatever the states are, and starts no session. `--stamp` re-stamps them without planning, `--output=json` gives each reference's `kind`, `line`, `state` and `fingerprint`, and a missing copy is refused with the `plan create --issue=<n>` that would write one.
- doctor: `rafa doctor` gains a row counting suspect, dangling and unknown references across every saved spec copy, naming `rafa issue check <n>` for each copy that holds one. An unreadable board counts as unknown instead of failing the row.
- Roadmap: `rafa roadmap` and `rafa issue list --roadmap` gain a `refs` column with the count of suspect and dangling references in each line's saved copy (`0` when clean, `-` with no copy, `?` when the copy could not be read), in text and JSON.
- triage: bugs the loop files carry a `## Refs` section after `## Artifact` listing the paths and symbols the artifact names, each stamped with its fingerprint at filing time, and omitted when the artifact names none. The `qa-bug-reporter` agent's guidance describes the section.
- Docs: `context/source.md` covers `src/refs/`, `context/cli.md` covers `rafa issue check` and the doctor row, `context/pull-requests.md` and `docs/specs-and-roadmap.md` describe check 4, and the README ticks the roadmap item "Verified and stamped references in specs".
